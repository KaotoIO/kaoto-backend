...
public interface WithConfigurationProperties {
    ...
    default boolean isEndpointProperty(Map.Entry<String, String> e) {
        return this.getProperties() != null
                ...
                && !this.getProperties().get(e.getKey()).componentProperty();
    }
    ...
    default boolean isSecret(String key) {
        return this.getProperties() != null
                && this.getProperties().containsKey(key)
                && this.getProperties().get(key).secret();
    }
    ...
    default boolean isRaw(String key) {
        return this.getProperties() != null
                && this.getProperties().containsKey(key)
                && this.getProperties().get(key).raw();
    }
    ...
    default Map<String, String> filterProperties(Map<String, String> properties,
                                                 Predicate<Map.Entry<String, String>> predicate) {
        return filterProperties(properties, predicate, Map.Entry::getKey,
                Map.Entry::getValue);
    }

    default Map<String, String> filterEndpointProperties(
            WithConfiguredProperties withConfigurationProperties) {
        return filterProperties(
                withConfigurationProperties.getConfiguredProperties(),
                this::isEndpointProperty);
    }
    ...
    default Map<String, String> filterComponentProperties(
            WithConfiguredProperties withConfigurationProperties) {
        return filterProperties(
                withConfigurationProperties.getConfiguredProperties(),
     ...
     *
     * @param properties     The specified configuration.
     * @param valueConverter A {@link Function} that is applies to each
     ...
     */
    default Optional<Map.Entry<String, ConfigurationProperty>> propertyEntryTaggedWith(
            final String tag) {
        return getProperties().entrySet().stream()
                .filter(entry -> entry.getValue().getTags().contains(tag))
                .findFirst();
    }
     ...
     * @return First property tagged with the supplied tag, if any
     */
    default Optional<String> propertyTaggedWith(
            final Map<String, String> properties, final String tag) {
        return propertyEntryTaggedWith(tag).map(
                entry -> properties.get(entry.getKey()));
    }
