...
public interface ConnectorSettings extends WithName, WithProperties {
    ...
    static InputStream determineSpecificationStream(
            final String configuredSpecification) {
        if (configuredSpecification.startsWith("http")) {
            try {
                ...
                try (InputStream in = url.openStream()) {
                    ...
                    IllegalArgumentException | MalformedURLException ignore) {
    ...
    static ByteArrayInputStream streamSpecificationContent(
            final String configuredSpecification) {
        return new ByteArrayInputStream(
                configuredSpecification.getBytes(StandardCharsets.UTF_8));
    }
    ...
    default Optional<InputStream> getSpecification() {
        final String configuredSpecification =
                getConfiguredProperties().get("specification");

        if (configuredSpecification == null
                || configuredSpecification.isBlank()) {
            return Optional.empty();
        }

        return Optional.of(
                determineSpecificationStream(configuredSpecification));
    }
