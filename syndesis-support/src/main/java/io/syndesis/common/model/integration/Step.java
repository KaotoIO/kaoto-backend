...
public interface Step
        extends WithId<Step>, WithConfiguredProperties, WithDependencies,
        WithMetadata, Serializable {
    ...
    default Step updateInputDataShape(
            final Optional<DataShape> inputDataShape) {
        return getAction().map(
                        a -> builder().action(a.withInputDataShape(inputDataShape)))
                .map(ImmutableStep.Builder::build)
                .orElseThrow(() -> new IllegalStateException(
                        "Unable to update input data shape of non existing action"));
    }

    default Step updateOutputDataShape(
            final Optional<DataShape> outputDataShape) {
        return getAction().map(
                        a -> builder().action(a.withOutputDataShape(outputDataShape)))
                .map(ImmutableStep.Builder::build)
                .orElseThrow(() -> new IllegalStateException(
                        "Unable to update output data shape of non existing action"));
    }

    default boolean hasInputDataShape() {
        Optional<DataShape> maybeShape = inputDataShape();
        return maybeShape.isPresent()
                && maybeShape.get().getKind() != DataShapeKinds.NONE;
    }
    ...
    default boolean hasUnifiedJsonSchemaOutputShape() {
        ...
        if (stepAction.isPresent()) {
            Optional<DataShape> outputShape =
                    stepAction.get().getOutputDataShape();
            if (outputShape.isPresent() && outputShape.get().getKind()
                    == DataShapeKinds.JSON_SCHEMA) {
                return outputShape.get().getMetadata()
                        .entrySet()
                        .stream()
                        .anyMatch(entry -> entry.getKey()
                                .equals(DataShapeMetaData.UNIFIED));
            }
        }
