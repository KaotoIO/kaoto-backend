...
public interface DataShape extends Serializable, WithName, WithMetadata {
    ...
    default Optional<DataShape> findVariantByMeta(String key, String val) {
        return findVariant((DataShape ds) -> {
    ...
    class Builder extends ImmutableDataShape.Builder {
        ...
        private static boolean shouldCompress(Map<String, String> metadata) {
            return Boolean.parseBoolean(
                    metadata.get(DataShapeMetaData.SHOULD_COMPRESS))
                    && !Boolean.parseBoolean(metadata.getOrDefault(
                    DataShapeMetaData.IS_COMPRESSED,
        ...
        private static boolean shouldCompressAnyVariant(
                List<DataShape> variants) {
            return variants.stream().anyMatch(variant -> Boolean.parseBoolean(
                    variant.getMetadata()
                            .get(DataShapeMetaData.SHOULD_COMPRESS)));
        }
        ...
        public Builder decompress() {
            ...
            if (shouldDecompress(currentShape.getMetadata())) {
                byte[] decoded = Base64.getDecoder()
                        .decode(currentShape.getSpecification()
                                .getBytes(StandardCharsets.UTF_8));
                try (ByteArrayInputStream zipped = new ByteArrayInputStream(
                        decoded);
                     GZIPInputStream is = new GZIPInputStream(zipped)) {
                    return new DataShape.Builder()
                            .createFrom(currentShape)
                            .putMetadata(DataShapeMetaData.IS_COMPRESSED,
                                    Boolean.FALSE.toString())
                    ...
                    throw new IllegalStateException(
                            "Failed to decompress data shape", e);
                }
        ...
        public Builder compress() {
            ...
            if (shouldCompress(currentShape.getMetadata())) {
                try (ByteArrayOutputStream zipped = new ByteArrayOutputStream();
                    ...
                    os.write(currentShape.getSpecification()
                            .getBytes(StandardCharsets.UTF_8));
                    os.finish();
                    ...
                    return new DataShape.Builder()
                            ...
                            .variants(currentShape.getVariants()
                                    ...
                                    .map(variant -> new DataShape.Builder()
                                            ...
                                            .build())
                                    .collect(Collectors.toList()))
                            .specification(Base64.getEncoder()
                                    .encodeToString(zipped.toByteArray()));
                } catch (IOException e) {
                    throw new IllegalStateException(
                            "Failed to compress data shape", e);
                }
            } else if (shouldCompressAnyVariant(currentShape.getVariants())) {
                return new DataShape.Builder()
                        .createFrom(currentShape)
