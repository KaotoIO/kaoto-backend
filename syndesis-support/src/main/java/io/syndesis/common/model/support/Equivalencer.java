...
public class Equivalencer {
    ...
    private boolean push(String id, Class<?> klazz, String failingProperty,
                         Object a, Object b) {
        if (failureContext == null) {
            failureContext = new ArrayDeque<>();
        }
    ...
    private boolean compare(String objectName, Class<?> objectKind,
                            EquivPair... pairs) {
    ...
    public String failureMessage() {
        ...
        while (iterator.hasNext()) {
            ...
            if (ctx.hasFailed()) {
                ...
                context.append(ctx.id()).append(" ").append("/").append(" ");
            }
        }
    ...
    private <T> boolean equivalent(List<T> oneList, List<T> anotherList,
    ...
    public boolean equivalent(Step one, Step another) {
        ...
        if (one == null) {
            return push(another.getName(), another.getClass(),
                    another.getName(), NULL, another.getName());
        }

        if (another == null) {
            return push(one.getName(), one.getClass(), one.getName(),
                    one.getName(), NULL);
        }

        if (!equivalent(one.getConnection().orElse(null),
                another.getConnection().orElse(null))) {
            return push(one.getName(), one.getClass());
        }

        if (!equivalent(one.getExtension().orElse(null),
                another.getExtension().orElse(null))) {
            return push(one.getName(), one.getClass());
        }

        if (!equivalent(one.getAction().orElse(null),
                another.getAction().orElse(null))) {
            return push(one.getName(), one.getClass());
        }
        ...
        return compare(one.getName(), one.getClass(),
                ...
                pair(one.getConfiguredProperties(),
                        another.getConfiguredProperties(),
                        CONFIGURED_PROPERTIES),
                pair(one.getDependencies(), another.getDependencies(),
                        DEPENDENCIES),
    ...
    public boolean equivalent(Extension one, Extension another) {
        ...
        return compare(one.getName(), one.getClass(),
                pair(one.getExtensionId(), another.getExtensionId(),
                        "extension-id"),
                pair(one.getSchemaVersion(), another.getSchemaVersion(),
                        "schema-version"),
                pair(one.getStatus(), another.getStatus(), "status"),
                pair(one.getIcon(), another.getIcon(), "icon"),
                pair(one.getDescription(), another.getDescription(),
                        DESCRIPTION),
                pair(one.getUserId(), another.getUserId(), "user-id"),
                pair(one.getExtensionType(), another.getExtensionType(),
                        "extension-type"),
                pair(one.getId(), another.getId(), ID),
                pair(one.getName(), another.getName(), NAME),
                pair(one.getTags(), another.getTags(), "tags"),
                pair(one.getProperties(), another.getProperties(),
                        "properties"),
                pair(one.getConfiguredProperties(),
                        another.getConfiguredProperties(),
                        CONFIGURED_PROPERTIES),
    ...
    public boolean equivalent(Integration one, Integration another) {
        ...
        if (!equivalent(one.getConnections(), another.getConnections(),
                Connection.class)) {
            return push(one.getName(), one.getClass());
        }
    ...
    public boolean equivalent(Connection one, Connection another) {
        ...
        if (!equivalent(one.getConnector().orElse(null),
                another.getConnector().orElse(null))) {
            return push(one.getName(), one.getClass());
        }

        return compare(one.getName(), one.getClass(),
                pair(one.getId(), another.getId(), ID),
                pair(one.getOrganization(), another.getOrganization(),
                        "organization"),
                pair(one.getOrganizationId(), another.getOrganizationId(),
                        "organization-id"),
                pair(one.getConnectorId(), another.getConnectorId(),
                        "connector-id"),
    ...
    public boolean equivalent(Connector one, Connector another) {
        ...
        if (!equivalent(one.getActions(), another.getActions(),
                ConnectorAction.class)) {
            return push(one.getName(), one.getClass());
        }

        return compare(one.getName(), one.getClass(),
                pair(one.getConnectorGroup(), another.getConnectorGroup(),
                        "connector-group"),
                pair(one.getConnectorGroupId(), another.getConnectorGroupId(),
                        "connector-group-id"),
                pair(one.getDescription(), another.getDescription(),
                        DESCRIPTION),
                pair(one.getIcon(), another.getIcon(), "icon"),
                pair(one.getKind(), another.getKind(), "kind"),
                pair(one.getComponentScheme(), another.getComponentScheme(),
                        "component-schema"),
                pair(one.getConnectorFactory(), another.getConnectorFactory(),
                        "connector-factory"),
                pair(one.getConnectorCustomizers(),
                        another.getConnectorCustomizers(),
                        "connector-customizers"),
                pair(one.getExceptionHandler(), another.getExceptionHandler(),
                        "exception-handler"),
                pair(one.getId(), another.getId(), ID),
                pair(one.getTags(), another.getTags(), "tags"),
                pair(one.getName(), another.getName(), NAME),
    ...
    public boolean equivalent(Action one, Action another) {
        ...
        if (!one.getClass().equals(another.getClass())) {
            return push(one.getName(), one.getClass(), "action-classes",
    ...
    public boolean equivalent(StepAction one, StepAction another) {
        ...
        return compare(one.getName(), one.getClass(),
                pair(one.getActionType(), another.getActionType(),
                        "action-type"),
                pair(one.getDescription(), another.getDescription(),
                        DESCRIPTION),
    ...
    public boolean equivalent(StepDescriptor one, StepDescriptor another) {
        ...
        if (one == null) {
            return push(another.getEntrypoint(), another.getClass(),
                    another.getEntrypoint(), NULL, another.getEntrypoint());
        }

        if (another == null) {
            return push(one.getEntrypoint(), one.getClass(),
                    one.getEntrypoint(), one.getEntrypoint(), NULL);
        }

        return compare(one.getEntrypoint(), one.getClass(),
                pair(one.getKind(), another.getKind(), "kind"),
                pair(one.getEntrypoint(), another.getEntrypoint(),
                        "entry-point"),
                pair(one.getResource(), another.getResource(), "resource"),
                pair(one.getInputDataShape(), another.getInputDataShape(),
                        "input-data-shape"),
                pair(one.getOutputDataShape(), another.getOutputDataShape(),
                        "output-data-shape"),
                pair(one.getPropertyDefinitionSteps(),
                        another.getPropertyDefinitionSteps(),
                        "property-definition-steps"));
    }
     ...
     */
    @SuppressWarnings("PMD.CompareObjectsWithEquals")
    public boolean equivalent(ConnectorDescriptor one,
        ...
        if (one == null) {
            ...
            return push(identifier, another.getClass(), identifier, NULL,
                    identifier);
        }

        String identifier = one.getConnectorId() + "-Descriptor";
        if (another == null) {
            return push(identifier, one.getClass(), identifier, identifier,
                    NULL);
        }
        ...
        return compare(identifier, one.getClass(),
                pair(one.getConnectorId(), another.getConnectorId(),
                        "connector-id"),
                pair(one.getCamelConnectorGAV(), another.getCamelConnectorGAV(),
                        "camel-connector-gav"),
                pair(one.getCamelConnectorPrefix(),
                        another.getCamelConnectorPrefix(),
                        "camel-connector-prefix"),
                pair(one.getComponentScheme(), another.getComponentScheme(),
                        "component-scheme"),
                pair(one.getConnectorFactory(), another.getConnectorFactory(),
                        "connector-factory"),
